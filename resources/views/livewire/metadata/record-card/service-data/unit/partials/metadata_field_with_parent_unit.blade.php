<div>
    @if ($updateMode)
        <x-metadata.update title="{{$title}}">
            <x-slot name="updateFields">
                @include('livewire.metadata.record-card.service-data.unit.partials.create_and_update_form_fields_with_parent_unit')
            </x-slot>
        </x-metadata.update>
    @else
    ...
    <x-metadata.metadata-component title="{{$title}}">
        <x-slot name="filter">
            <div wire:ignore>
            <select class="form-control custom-select"  wire:model="filter">
        ...
        <x-slot name="tableData">
            @foreach($data as $row)
                <tr>
                    ...
                    <td>{{$row->name}}</td>
                    <td>{{$row->slug}}</td>
                    <td>{{$row->battalion->slug ?? $row->company->slug ?? ''}}</td>
