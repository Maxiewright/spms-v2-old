<div>
    @if ($updateMode)
        <x-metadata.update title="{{$title}}">
            <x-slot name="updateFields">
                ...
                @include('livewire.metadata.record-card.extracurricular.partials.create_and_update_form_fields')
            </x-slot>
        </x-metadata.update>
    @else
    ...
    <x-metadata.metadata-component title="{{$title}}">
        <x-slot name="filter">
            <div wire:ignore>
                <select wire:model="filter" class="form-control custom-select mb-2 mr-sm-2">
