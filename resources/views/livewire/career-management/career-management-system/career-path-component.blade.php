    ...
    @if ($updateMode)
        <x-metadata.update title="{{$title}}">
            <x-slot name="updateFields">
                @include('livewire.career-management.career-management-system.partials.career_path_create_and_update_form_fields')
            </x-slot>
        </x-metadata.update>
    @else
...
@push('livewire-scripts')
    <script>
        livewireDeleteConfirmation('career_path_destroy','career_path')
    </script>
@endpush
