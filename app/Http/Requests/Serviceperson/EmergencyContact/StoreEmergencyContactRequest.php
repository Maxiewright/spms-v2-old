...
class StoreEmergencyContactRequest extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            'employer'        => 'nullable',
            'is_primary'      => [
                Rule::unique('emergency_contacts', 'is_primary')->where(function ($query){
                    $query->where('serviceperson_number', session('serviceNumber'));
                })
            ],
//            Address
            'address1'        =>  'required',
...
//            Primary Contact
            'is_primary.unique'          => 'A Type of contact already exist for this serviceperson'
        ];
    }
}
